...
int main() {
	...
	// open 
	if((fd = open("/dev/CDD2", O_TRUNC | O_RDWR)) == -1) {
	...
	// Should succeed because only Eureka! is only 7 chars long.
	if (lseek(fd, 7, SEEK_SET) == -1) {
		fprintf(stderr, "ERR: on lseek(): %s\n", strerror(errno));
	}

	// Should fail because only Eureka! is only 7 chars long.
	if (lseek(fd, 8, SEEK_SET) == -1) {
		fprintf(stderr, "Cannot lseek 8. Eureka is only 7 chars long: %s\n", strerror(errno));
	}
	close(fd);
}
